  ...
  const handleSubmit = async () => {
    try {
      ...
      const billBlob = uploadedData.bill
        ...
        : null;
      const clothesBlob = uploadedData.clothes
        ? await convertToBlob(uploadedData.clothes)
        : null;
      ...
      if (clothesBlob) formData.append("item", clothesBlob);
      ...
      {viewUploadedImages ? (
        ...
        <>
          ...
          <div className="flex flex-col w-full max-w-md space-y-4 px-4 mb-6 mt-6">
            ...
            <label
              className={`mx-auto w-3/4 py-3 ${
                uploadedStatus.clothes ? "bg-green-500" : "bg-[#407daa]"
              } text-white rounded-full text-sm font-semibold hover:bg-blue-700 flex items-center justify-center gap-2 cursor-pointer`}
            >
              <Upload size={20} />
              Upload Clothes
...
export default ClothesMobileComponent;
