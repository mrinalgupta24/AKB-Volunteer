...
const MedicineMobileComponent = () => {
  ...
  useEffect(() => {
    const fetchDonorInfo = async () => {
      try {
        const response = await api.get("/api/donor_info/", {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const billBlob = uploadedData.bill
        ...
        : null;
      const medicineBlob = uploadedData.medicine
        ? await convertToBlob(uploadedData.medicine)
        : null;
      ...
      if (medicineBlob) formData.append("item", medicineBlob);
      ...
      {viewUploadedImages ? (
        ...
        <>
          ...
          <div className="flex flex-col w-full max-w-md space-y-4 px-4 mb-6 mt-6">
            ...
            <label
              className={`mx-auto w-3/4 py-3 ${
                uploadedStatus.medicine ? "bg-green-500" : "bg-[#407daa]"
              } text-white rounded-full text-sm font-semibold hover:bg-blue-700 flex items-center justify-center gap-2 cursor-pointer`}
            >
              <Upload size={20} />
              Upload Medicine
              <input
                type="file"
                className="hidden"
                onChange={(e) => handleFileUpload(e, "medicine")}
              />
            </label>
            <button
              className={`mx-auto w-3/4 py-3 ${
...
export default MedicineMobileComponent;
